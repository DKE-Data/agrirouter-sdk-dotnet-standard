...
namespace Agrirouter.Api.Test.Data
{
    ...
    public class OnboardResponseIntegrationService
    {
        ...
        /// <summary>
        /// Write an onboarding response to the file.
        /// </summary>
        /// <param name="onboardResponse">Onboarding response to write to the file.</param>
        /// <param name="identifier">Identifier.</param>
        public static void Write(OnboardResponse onboardResponse, string identifier)
        {
            var path = PathToWrite(identifier);
            var serializedObject = JsonConvert.SerializeObject(onboardResponse);
            File.WriteAllBytes(path, Encoding.UTF8.GetBytes(serializedObject));
        }

        private static string PathToWrite(string identifier)
        {
            var path = System.IO.Path.Combine(
                System.IO.Path.GetDirectoryName(@"Data/OnboardingResponses/") ??
                throw new InvalidOperationException(), identifier + ".json");
            return path;
        }
    }
}