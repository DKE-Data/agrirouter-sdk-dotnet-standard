using System;
using System.Net.Http;
using System.Security.Cryptography.X509Certificates;
using Agrirouter.Api.Dto.Onboard;
using Agrirouter.Api.Exception;
using Agrirouter.Api.Logging;
using Agrirouter.Impl.Service.messaging;
using Newtonsoft.Json;
using Xunit;

namespace Agrirouter.Api.test.service.messaging
{
    public class FetchMessagesServiceTest
    {
        [Fact]
        public void GivenInvalidOnboardingResponseWhenFetchingMessagesThenTheResponseShouldBeValid()
        {
            var httpClient = AuthenticatedHttpClient(InvalidOnboardingResponse);
            var fetchMessageService = new FetchMessageService(httpClient);
            Assert.Throws<CouldNotFetchMessagesException>(() => fetchMessageService.Fetch(InvalidOnboardingResponse));
        }

        [Fact]
        public void GivenValidOnboardingResponseWhenFetchingMessagesThenTheResponseShouldBeValid()
        {
            var httpClient = AuthenticatedHttpClient(ValidOnboardingResponse);
            var fetchMessageService = new FetchMessageService(httpClient);
            Assert.Empty(fetchMessageService.Fetch(ValidOnboardingResponse));
        }

        private static OnboardingResponse InvalidOnboardingResponse
        {
            get
            {
                var onboardingResponseAsJson =
                    "{\"deviceAlternateId\": \"f67f2e54-c9bf-4f12-813c-2e8f0df075e4\",\"capabilityAlternateId\": \"800d79ca-f54b-42f1-a683-15724a4cfaeb\",\"sensorAlternateId\": \"d3876788-36c3-4128-871c-18a8a3f340f5\",\"connectionCriteria\":{\"gatewayId\":\"3\",\"measures\": \"https://dke-qa.eu10.cp.iot.sap/iot/gateway/rest/measures/f67f2e54-c9bf-4f12-813c-2e8f0df075e4\",\"commands\": \"https://dke-qa.eu10.cp.iot.sap/iot/gateway/rest/commands/f67f2e54-c9bf-4f12-813c-2e8f0df075e4\"},\"authentication\": {\"type\": \"P12\",\"secret\": \"Teqrkqobmxzv1TMW65sKVupVJATTHlv1EJsi\",\"certificate\": \"\"}}";
                var onboardingResponse =
                    JsonConvert.DeserializeObject(onboardingResponseAsJson, typeof(OnboardingResponse));
                return onboardingResponse as OnboardingResponse;
            }
        }

        private static OnboardingResponse ValidOnboardingResponse
        {
            get
            {
                var onboardingResponseAsJson =
                    "{\"deviceAlternateId\":\"44d7003e-2743-492b-86aa-4fed91fcf20a\",\"capabilityAlternateId\":\"c2467f6d-0a7e-48ca-9b57-1862186aef12\",\"sensorAlternateId\":\"bfbfa1a5-bf09-4423-9ad3-3678368ffe53\",\"connectionCriteria\":{\"gatewayId\":\"3\",\"measures\":\"https://dke-qa.eu10.cp.iot.sap/iot/gateway/rest/measures/44d7003e-2743-492b-86aa-4fed91fcf20a\",\"commands\":\"https://dke-qa.eu10.cp.iot.sap/iot/gateway/rest/commands/44d7003e-2743-492b-86aa-4fed91fcf20a\"},\"authentication\":{\"type\":\"P12\",\"secret\":\"8A71w6kCuckYiTK4R1k4bCVxsnB6flcK9crZ\",\"certificate\":\"\"}}";
                var onboardingResponse =
                    JsonConvert.DeserializeObject(onboardingResponseAsJson, typeof(OnboardingResponse));
                return onboardingResponse as OnboardingResponse;
            }
        }

        /// <summary>
        /// Create a single HTTP client using the given onboarding response.
        /// </summary>
        /// <param name="onboardingResponse">The current onboarding response.</param>
        /// <returns>-</returns>
        public static HttpClient AuthenticatedHttpClient(OnboardingResponse onboardingResponse)
        {
            var httpClientHandler = new HttpClientHandler();
            httpClientHandler.ClientCertificates.Add(new X509Certificate2(
                Convert.FromBase64String(onboardingResponse.Authentication.Certificate),
                onboardingResponse.Authentication.Secret));
            var httpClient = new HttpClient(new LoggingHandler(httpClientHandler));
            return httpClient;
        }
    }
}